...
exports.updateWeeklyPerformance = async (req, res) => {
  try {
    const weekly_Quiz_id = req.params.WeeklyQuizid;
    console.log(weekly_Quiz_id);
    console.log(req.body);
    const {
      StudentId,
      Weekstartingdate,
...
exports.Create_Progress = catchAsyncErrors(async (req, res) => {
  try {
    console.log(req.body);
    const newProgress = new Progress({
      StudentId: req.body.id,
      QuestionsWrong: req.body.WrongQuestions,
      QuestionsCorrect: req.body.CorrectQuestions
    });
    await newProgress.save();
    res.status(200).json({ success: true, newProgress });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
